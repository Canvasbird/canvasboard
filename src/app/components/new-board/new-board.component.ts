...
import { AddCanvasBoard } from 'src/app/plugins/cb-canvasboard';
...
export class NewBoardComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    public nextActiveMatch() {
        this.activeIndex =
            this.activeIndex < $('#search_results li').length - 1
                ? ++this.activeIndex
                : this.activeIndex;
    }
        ...
        switch (idFilter) {
            ...
            case 16: {
                this.cbToolbox(
                    this.AddEmbedComponent,
                    'embed',
                    'https://node-event-loop.herokuapp.com/'
                );
                break;
            }
            case 17: {
    ...
    async saveData() {
        ...
        ids.forEach((key) => {
            ...
            if (
                ...
                ele.setContent(
                    this.AddMarkDownComponent.getContent(ele.cardID)
                );
            }
        ...
        if (
            ...
            this.fileID = (
                await this.apiService.createBoardData(createDataJson)
            )._id;
        }
    ...
    populateData(data: NewBoard | Params) {
        ...
        data.data.forEach((element, index) => {
            ...
            if (index === 0) {
                ...
                $(`#cb-box-1-${prevId}`).after(
                    this.blockFunction(element.cardID)
                );
            }
            ...
            switch (element.pluginType) {
                ...
                case 'board': {
                    this.AddCanvasBoard.setContent(
                        element.cardID,
                        element.content
                    );
                    break;
                ...
                case 'embed': {
                    this.AddEmbedComponent.addToolBox(
                        element.cardID,
                        element.content
                    );
                    break;
                ...
                case 'tweet': {
                    this.AddTwitterComponent.addToolBox(
                        element.cardID,
                        element.content
                    );
                    break;
                ...
                case 'markdown': {
                    this.AddMarkDownComponent.setContent(
                        element.cardID,
                        element.content
                    );
                    break;
    ...
    cbToolboxTopTag = () => {
        this.AddTopComponent.addToolBox(
            this.currentChartID,
            this.addBlockEditor
        );
    };
